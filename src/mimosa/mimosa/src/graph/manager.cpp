...
namespace graph
{
Manager::Manager(rclcpp::Node & pnh, mimosa::imu::Manager::Ptr imu_manager)
: config_(config::checkValid(config::fromRos<ManagerConfig>(pnh))),
  imu_manager_(imu_manager),
  isam2_params_(getISAM2Params()),
  tf2_broadcaster_(pnh),
  tf2_static_broadcaster_(pnh)
{
  // Prepare logger
  logger_ =
  ...
  pub_debug_ = pnh.create_publisher<mimosa_msgs::msg::GraphManagerDebug>("graph/debug", 1);
  pub_path_ = pnh.create_publisher<nav_msgs::msg::Path>("graph/path", 1);
  pub_odometry_ = pnh.create_publisher<nav_msgs::msg::Odometry>("graph/odometry", 1);
  pub_transform_stamped_ = pnh.create_publisher<geometry_msgs::msg::TransformStamped>("graph/transform", 1);
  pub_optimized_path_ = pnh.create_publisher<nav_msgs::msg::Path>("graph/optimized_path", 1);
}
...
void Manager::publishResults()
{
  ...
  if (pub_odometry_->get_subscription_count()) {
    ...
    odometry.header.stamp = toStamp(state_.ts());
  ...
  for (const auto & [key, value] : optimized_values_) {
    ...
    pose.header.stamp = toStamp(state_.ts());
